...
frappe.ui.form.on('Sale Order', {
	onload: function(frm){
		frappe.call({
			...
			callback: function(res){
				frm.set_value("order_date", res.message[1]);
				frm.refresh_field("order_date");
			}
		})
	},
	customer:function(frm){
		if(frm.doc.customer){
			frappe.call({
				method:"textile.textile.doctype.sale_order.sale_order.customer_count",
				args:{
...
frappe.ui.form.on('Estimate Item',{
	item_name:function(frm, cdt, cdn){
		...
		if(!frm.doc.items[index].item_name){
			...
			frm.set_value("total_price", amount);
